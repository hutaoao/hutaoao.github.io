...
**<font style="color:rgb(0, 0, 0);">概念：</font>\*\*\*\*<font style="color:#DF2A3F;">即在调用函数时，函数不带任何修饰，就会应用默认绑定规则, 默认绑定指向全局作用域。</font>**
...
**概念：\*\*\*\*<font style="color:#DF2A3F;">当函数在调用时，如果函数有所谓的“落脚点”，即有上下文对象（即调用时.前面的对象）时，隐式绑定规则会把函数中的 this 绑定到这个上下文对象。</font>**
...
下面那种：obj.m 只是一个函数地址，不是函数本身。所以调用时，还是在当前全局环境下调用。
...
\*\*概念：\*\*通过构建函数`new`关键字生成一个实例对象，此时`this`指向这个实例对象
...
**！！！**<code>**new**</code>**过程遇到**<code>**return**</code>**一个对象，此时**<code>**this**</code>**指向为返回的对象**
...
<code>**apply()、call()、bind()**</code>**是函数的一个方法，作用是改变函数的调用对象**。它的第一个参数就表示改变后的调用这个函数的对象。因此，这时`this`指的就是这第一个参数
...
> 更新: 2024-01-10 16:12:04  
> 原文: <https://www.yuque.com/hutaoao/blog/tynael1nazhl4zd9>