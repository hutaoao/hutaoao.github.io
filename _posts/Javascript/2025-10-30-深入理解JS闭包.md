...
所以，在本质上，<font style="color:#DF2A3F;">闭包就是将函数内部和函数外部连接的一座桥梁</font>。
...
function f1(){
　　var n = 999;
　　nAdd = function(){ // 没有使用var关键字，因此nAdd是一个全局变量，而不是局部变量
　　　　n += 1;
　　}
　　function f2(){
　　　　console.log(n);
...
<body>
    ...
    <script>
        ...
        const fn = add();
        function myFunction() {
            document.getElementById("demo").innerHTML = fn();
        }*/
...
变量 add 的赋值是自执行函数的返回值。

这个自执行函数只运行一次。它设置计数器为零（0），并返回函数表达式。
...
<font style="color:#DF2A3F;">闭包指的是有权访问父作用域的函数，即使在父函数关闭之后。</font>



### 六、使用闭包的注意点
1. 由于闭包会使得函数中的变量都被保存在内存中，内存消耗很大，所以不能滥用闭包，否则会造成网页的性能问题，在IE中可能<font style="color:#DF2A3F;">导致内存泄露</font>。解决方法时，在退出函数之前，将不使用的局部变量全部删除。
2. 闭包会在父函数外部，改变父函数内部变量的值。所以，如果你把父函数当作对象(object)使用，把闭包当作它的公用方法，把内部变量当作它的私有属性，这时一定要小心，不要随便改变父函数内部变量的值。



> 更新: 2024-01-18 13:23:09  
> 原文: <https://www.yuque.com/hutaoao/blog/wirnmg>