...
> <code>BigInt</code>进行**任意精度的整数算术**：可以用来表示或操作任意大的整数，只受限于系统的可用内存

> 注：Function、Array都继承于Object，如下图：

![1616295088107-caa3eea7-8477-4d6b-96b1-817e26ccc974.png](/assets/img/posts/知识积累/面试题/1616295088107-caa3eea7-8477-4d6b-96b1-817e26ccc974-742749.png)
...
> 主要在于它们在\*\*内存中的存储方式**以及**比较方式**。基本类型直接**存储值本身**，而引用类型存储的是指向实际数据的**内存地址的引用（堆）\*\*。同时，比较基本类型值时，比较的是它们的值，而比较应用类型值时，比较的是它们的引用。
...
3. **Object.prototype.toString -**** 最佳（ES6之前）**** - **可以**精确的判断各种数据类型**
...
4. **⚠️**\*\*判断数组最佳方案 Array.isArray()  ****👍**** ****👍**** ****👍**** ****👍**** ****👍**** \*\*\*\*👍\*\*

***

### JSON 的了解

<code>**JSON(JavaScript Object Notation)**</code> 是一种轻量级的数据交换格式。
...
* <code>**defer**</code> 用于<code><script></code>标签的属性，它指示浏览器延迟脚本的加载和执行，直到整个HTML文档都解析完成
...
<code>**null**</code> 表示一个**尚未存在的对象**，转换为数值时为0；

<code>**undefined**</code> 表示**声明的变量未初始化**，转换为数值时为NAN；
...
又或者：函数A 里面包含了 函数B，而 函数B 里面使用了 函数A 的变量，那么 函数B 被称为闭包。
...
2. 所有末定义直接赋值的变量自动声明为拥有全局作用域(不论函数内外)

* **函数作用域：**在固定的代码片段才能被访问
* \*\*块级作用域：\*\***let/const**，在大括号中使用let和const声明的变量存在于块级作用域中。在大括号之外不能访问这些变量

![1704697631493-8cf25653-b2ae-4bf3-8613-cd8c3c4ec8df.png](/assets/img/posts/知识积累/面试题/1704697631493-8cf25653-b2ae-4bf3-8613-cd8c3c4ec8df-574468.png)
...
当使用一个变量的时候：首先js引擎会在当前作用域下去寻找，如果没找到 就会向上级作用域去查，直到找到该变量或查到全局作用域，这么一个查找过程形成的链条就叫做**作用域链**

![1704697807422-bd83a09b-8bfe-48df-aa14-0502eec44cd8.png](/assets/img/posts/知识积累/面试题/1704697807422-bd83a09b-8bfe-48df-aa14-0502eec44cd8-978286.png)
...
所有实例对象都有一个\*\* \*\*<code>__proto__</code> 属性，这个属性指向它的原型对象，所以实例对象都会继承这个原型对象的属性和方法；

原型对象（prototype）上也有一个\*\* ****proto**** **属性 指向 Object 原型对象，而 Object 原型对象的** ****proto**** \*\*指向的是 null。

当我们访问对象的某个属性时，就会从实例对象，原型对象，Object原型对象上层层寻找，形成的链式结构称为**原型链**。
...
| GET | POST |
| --- | --- |
| **参数URL可见**<br/>+ 参数保留在浏览器历史中<br/>+ 能被缓存<br/>+ 相对安全性较差 | 参数URL不可见，存放在HTTP的包体内<br/>+ 参数不会保存在浏览器历史中<br/>+ 不能缓存<br/>+ 相对安全性较好 |
| URL长度有限制（浏览器限制） | 无限制 |
...
> * post 不同，post做的一般是修改和删除的工作，所以必须与数据库交互，所以**不能使用缓存**。因此get请求适合于请求缓存。
...
<code>target</code>和<code>currentTarget</code>都是事件对象（Event Object）中的属性

* **target** 属性代表触发事件的DOM元素；在事件流的目标阶段；不会随着事件在DOM树中的传递而改变。
* **currentTarget** 属性表示当前正在处理事件的元素，这可能是事件的源头，也可能不是；在事件流的捕获、目标及冒泡阶段；可能会随着事件在DOM树中的传递而改变。

> 如果你只关心事件最初发生的元素，可以使用target；
> 如果你需要跟踪事件在整个DOM树中的传递过程，可以使用currentTarget。

***

### export和export default的区别

1. **数量不同**：export可以导出多个；export default只能默认的，只能有一个。
2. **使用时的语法不同**：export default导入导出时不用加<code>{}</code>；export 导入导出一般 要加<code>{}</code>。
...
指的是通过在一段时间内保留已接收到的web资源的一个副本。如果发起了对这个资源的再一次请求，那么浏览器会直接使用缓存的副本，而不是向服务器发起请求。

其机制是根据 <code>HTTP</code> 报文的缓存标识（头部信息）来控制缓存行为。
...
1. Ajax就是异步的 JS 和 XML（目前我们一般用JSON代替XML）
2. Ajax主要用于不刷新页面的情况下浏览器发起请求并接受响应，局部更新
3. Ajax核心点是 XMLHttpRequest 对象
...
**CORS **是一种新标准，支持同源通信，也支持跨域通信，fetch是实现CORS通信的。

CORS需要浏览器和服务器同时支持，实现CORS通信的关键是服务器。只要服务器实现了CORS接口（CORS 通过在服务器端设置特殊的 HTTP 头部信息来实现跨域资源共享），就可以跨源通信。
...
* <code>http</code> 是超文本传输协议，信息是明文传输，<code>https</code> 则是具有安全性的 <code>ssl</code> 加密传输协议
* `http` 和 `https` 使用的是完全不同的连接方式，用的端口也不同，前者是 `80` ，后者是 `443`
...
**相同点：**为了改变函数体内部 this 的指向。

**不同点：**

参数书写方式不同：call(thisObj, arg1, arg2, arg3)；apply(thisObj, \[arg1, arg2, arg3])；

thisObj：call 和apply 第一个参数都是 this 的指向对象；
...
```
- **核心对象**：如Object, Function, Array, String, Boolean, Number, Date, RegExp, Error等。
- **数学对象**（Math）：提供了一系列数学相关的方法和常量。
- **JSON对象**（JSON）：用于处理JSON数据。
- **其他内置对象**：如decodeURI, encodeURI, decodeURIComponent, encodeURIComponent等全局函数。
```
...
```
- **全局对象**（在浏览器中是window，在Node.js中是global）：代表了全局作用域，并提供了许多全局函数和变量。
- **全局函数**：如eval(), isNaN(), parseFloat(), parseInt()等。
- **其他构造函数**：如Array, Function, Date, RegExp等，它们不仅是构造函数，也是内置对象。
```
...
![1616297039688-45d8b62b-6418-489a-a125-badf3c3d5970.png](/assets/img/posts/知识积累/面试题/1616297039688-45d8b62b-6418-489a-a125-badf3c3d5970-995457.png)

2. 如果有new关键字，this 指向 new 出来的那个对象

![1616297662793-4cd714af-8e38-4aec-8dbf-b1e7bd466ede.png](/assets/img/posts/知识积累/面试题/1616297662793-4cd714af-8e38-4aec-8dbf-b1e7bd466ede-054787.png)

3. 在DOM 事件中，this 指向目标元素
4. 箭头函数的 this 指向他所在的函数级作用域，并且不可改变

![1616297145622-d3b28023-dd77-4e92-a038-ac310602c45c.png](/assets/img/posts/知识积累/面试题/1616297145622-d3b28023-dd77-4e92-a038-ac310602c45c-734596.png)
...
1. 创建一个空对象（即{ } ）；
2. 设置原型链：这个新创建的对象的\[\[Prototype]]（原型）被设置为构造函数的prototype对象，这使得新对象可以继承原型的属性和方法。
3. 绑定构造函数：new操作符将当前上下文绑定到构造函数上，这意味着在构造函数中this关键字将指向新创建的对象（改变this指向）
4. 返回新对象：如果构造函数成功地返回了一个对象，那么这个对象就会被返回。否则，new操作符会返回undefined。
...
**函数声明：\*\*\*\*变量提升 - **使用<code>function</code>关键字来声明一个函数
...
![1616304350522-5ce17ae8-5123-4224-b81e-233cab9cc51a.png](/assets/img/posts/知识积累/面试题/1616304350522-5ce17ae8-5123-4224-b81e-233cab9cc51a-209551.png)

3. 消除 js 不合理，不严谨地方，减少怪异行为

![1616304406444-48f63444-a657-4c84-918b-113a3a91a2be.png](/assets/img/posts/知识积累/面试题/1616304406444-48f63444-a657-4c84-918b-113a3a91a2be-744220.png)
...
### eval 解释器
 ...
 是 JavaScript 中的一个**内置函数**，它可以**将传入的字符串作为 JavaScript 代码进行解析和执行**。

1. 正因为如此，他的**执行效率特别慢**，因为要编译一次然后执行一次
2. eval 中声明的**变量是不会被提升**的

![1616305041510-5de45769-f4f9-4eed-a5aa-e4d684290a38.png](/assets/img/posts/知识积累/面试题/1616305041510-5de45769-f4f9-4eed-a5aa-e4d684290a38-387673.png)
...
**浅拷贝（****Shallow Copy****）**：浅拷贝只是拷贝一层，复制对象的引用 - 意味着复制的对象会受到原始对象的影响

1. for···in：只循环第一层
2. Object.assign(target, ...sources)

**深拷贝（Deep Copy）**：复制对象的所有层级和嵌套数据，创建完全独立的副本 - 意味着复制的对象不会受到原始对象的影响
...
4. Array 的 slice 和 concat 方法 返回新数组
...
**同步任务进入主线程，即主执行栈，异步任务进入任务队列，主线程内的任务执行完毕后，会去任务队列读取对应的任务，推入主线程执行。上述过程的不断重复就是【事件循环】**

***

### **JS执行机制 - 事件循环/2**

JavaScript 的事件循环（Event Loop）机制是 JavaScript 运行时环境（如浏览器或 Node.js）用来处理异步操作和回调的一种重要方式。它使得 JavaScript 能够在执行单线程代码的同时，仍然能够响应用户输入、网络请求等异步事件。

下面是 JavaScript 事件循环机制的基本工作原理：

1. **调用栈（Call Stack）**：
   * JavaScript 引擎有一个单一的线程，该线程有一个调用栈来跟踪在这个线程上执行的函数调用。调用栈是一种后进先出（LIFO）的数据结构，它记录了当前执行上下文的函数调用。
2. **同步代码执行**：
   * 当 JavaScript 代码开始执行时，它会按照代码的顺序，一行一行地将同步函数压入调用栈并执行。只有当调用栈为空时，JavaScript 引擎才会考虑执行异步任务。
3. **Web APIs**：
   * 浏览器提供了一些 Web APIs（如 setTimeout, fetch, addEventListener 等），这些 API 允许 JavaScript 与浏览器环境进行交互，并执行一些异步操作。当这些异步操作完成时，它们的结果会被添加到任务队列（Task Queue）或微任务队列（Microtask Queue）中等待处理。
4. **任务队列与微任务队列**：
   * 任务队列（Task Queue）和微任务队列（Microtask Queue）都是存放待处理任务的队列。任务队列通常存放宏任务（macrotask），如 setTimeout、setInterval、UI 渲染、用户交互事件等；而微任务队列存放微任务（microtask），如 Promise 的回调、process.nextTick（Node.js）等。
5. **事件循环**：
   * 当调用栈为空时，事件循环会检查微任务队列。如果微任务队列中有任务，事件循环会将其全部取出并执行，直到微任务队列为空。这个过程会不断重复，直到微任务队列中再也没有任务。
   * 微任务队列清空后，事件循环会检查任务队列。如果任务队列中有任务，它会取出一个任务并执行。这个任务可能是一个事件回调、一个 setTimeout 的回调等。执行这个任务时，如果有新的微任务产生，它们会被添加到微任务队列中，等待下一轮的事件循环处理。
   * 这个过程会一直循环下去，直到任务队列和微任务队列都为空，并且没有更多的异步任务产生。

通过这种方式，JavaScript 能够在不阻塞单线程的情况下处理异步操作，并且保证了异步操作的执行顺序（微任务先于宏任务，同一类型的任务按照它们被添加到队列的顺序执行）。

> 需要注意的是，虽然事件循环使得 JavaScript 能够处理异步操作，但过度使用或滥用异步操作也可能导致性能问题或难以调试的代码。
...
事件传播是双向的，从window到事件目标，然后再返回。可以分为三个阶段：（Capturing > Target > Bubbling）

* <code>Capturing：捕获阶段</code> — 事件捕获：从window到事件目标的父级元素
* <code>Target：目标阶段</code> — 事件目标，在此阶段，将调用在事件目标上注册的所有监听器，而不管其捕获标志的值如何。
* <code>Bubbling:冒泡阶段</code> — 事件冒泡：从事件目标的父级元素回到 window

> 停止事件传播：<code>event.stopPropagation()</code>
> 阻止默认事件传播：<code>event.preventDefault()</code> 》 - 阻止浏览器默认行为
...
* **Object.keys(obj)**：返回一个由对象的所有自身可枚举属性的**属性名组成的数组**。
* **Object.values(obj)**：返回一个由对象的所有自身可枚举**属性的值组成的数组**。
* **Object.entries(obj)**：返回一个由对象的所有自身可枚举**属性的键值对组成的数组**。
* **Object.getOwnPropertyNames(obj)**：返回一个由对象的所有自身属性（包括不可枚举属性但不包括Symbol值作为名称的属性）的属性名（包括不可枚举属性）组成的数组。
...
* **slice(start, end)：**返回被截取的部分；start（包含） 和 end（不包含）；第二个参数省略的话 截取到结尾
* **substr(start, length)：**从起始索引位置提取字符串中指定数目的字符；第二个参数省略的话 截取到结尾
* **substring(start, end)：**提取两个指定的索引号之间的字符
* **charAt(index)：**返回在指定位置的字符
* **charCodeAt(index)：**返回在指定的位置的字符的 Unicode 编码
* **indexOf(str)：**从开头去搜索传入的字符串，并返回位置（如果没找到，则返回 -1 ）
* **trim()、trimLeft()、trimRight()：**删除前、后或前后所有空格符，再返回新的字符串
* **repeat(count)：**复制字符串指定次数，并将它们连接在一起返回
* **padStart(length, str) / padEnd(length, str)：**头部/尾部补全，直至满足长度条件
* **toLowerCase() / toUpperCase()：**大写/小写转换
* **startWith(searchString, start)：**查看字符串是否以指定的子字符串开头，如果是返回 true，否则 false
* **includes(searchString, start)：**查找字符串中是否包含指定的子字符串，匹配到字符串则返回 true，否则返回 false
* **split(str)：**把字符串分割为字符串数组
* **match(regexp)：**查找找到一个或多个正则表达式的匹配，匹配到将返回一个数组，否则返回null
...
**应用场景**：（1）手机号、邮箱地址的校验 （2）当用户input框输入完成后再发请求，如搜索等
...
**应用场景**：高频事件，如：多少秒之后获取验证码、resize 事件和scroll 事件等
...
* **XSS** (Cross Site Scripting) 跨站脚本攻击 - 允许**攻击者（第三方）**将恶意代码植入到提供给其它用户使用的页面中
* **CSRF**（Cross-site request forgery）跨站请求伪造 - 攻击者诱导受害者进入**第三方网站**，在第三方网站中，向被攻击网站发送跨站请求
...
* **持久化**的本地存储，除非主动删除数据，否则数据是永远不会过期的
* 存储的信息在**同一域中是共享的**
* 当本页操作（新增、修改、删除）了localStorage的时候，本页面不会触发storage事件,但是别的页面会触发storage事件。
* 大小：**5M**（跟浏览器厂商有关系）
* localStorage本质上是对**字符串的读取**，如果存储内容多的话会消耗内存空间，会导致页面变卡
* **受同源策略的限制**
...
和 localStorage 使用方法基本一致，唯一不同的是生命周期，一旦页面（会话）关闭，sessionStorage 将会删除数据
...
* 没有及时清除事件监听 <code>addEventListener</code>
...
使用了<code>Set</code>对象的特性，它**只允许包含唯一值的集合**，通过将数组转化为Set，然后再将Set转回为数组，即可得到去重后的数组。
...
1. 使用 <code>reduce</code> 方法：

利用reduce方法迭代数组，将不重复的值添加到一个累加器数组中，然后利用 reduce。
...
1. 使用<code>filter</code>方法和<code>indexOf</code>：

使用filter方法和indexOf来检查每个元素是否在数组中的**第一个出现位置**，从而筛选出不重复的元素。
...
1. 使用<code>ES6</code>的<code>Map</code>：

使用ES6的Map对象，通过将数组元素映射成\[key, value]的形式，并将其转化为Map对象后，再通过扩展操作符将Map对象转回为数组。
...
* **性质不同**：Set是值的**集合**，Map是**键值对**。
* **存储方式不同**：Set以\[value, value]的形式储存元素，Map以\[key, value] （键值对）的形式储存。
* **获取方式不同**：Set通过\[]获取，Map通过**get()**获取。
* **用途不同**：Set主要应用在数组去重；Map由于没有格式限制，Map主要应用在数据存储。

***

### ES6新特性

<code>ES6</code> 是 JavaScript 语言的下一代标准，它的目标是使得 JavaScript 语言可以用来**编写复杂的大型应用程序**
...
**Promise 是****异步编程的****一种解决方案，ES6新标准（统一JS的异步实现方案）**

> 在promise之前，异步的实现都是通过回调进行的，回调每个人实现不一样，所以promise核心是统一JS的异步实现方案，才有 <code>async</code> <code>await</code>

有三种状态：pending（进行中）、fulfilled（已成功）和 rejected（已失败），一旦状态变为已成功或已失败，就不可再改变。

使用 <code>.then()</code> 方法处理异步操作成功的情况，使用 <code>.catch()</code> 方法处理异步操作失败的情况



**处理以下场景问题：**

1. 链式回调 - <code>async</code> <code>await</code>
2. 同时发起几个异步请求，谁先有结果就拿谁的 - <code>Promise.race(requests)</code>
3. 发起多个请求，等到所有请求后再做下一步处理 - <code>Promise.all(requests)</code>
...
### 什么是webpack，它的核心原理是什么

<code>Webpack</code> 是一个用于**现代 JS 应用程序的静态模块打包工具**，其核心原理如下：

* **把一切视为模块**：webpack通过一个开发时态的入口模块为起点，分析出所有的依赖关系，然后经过一系列的过程（压缩、合并），最终生成运行时态的文件。
* **构建依赖图**：当webpack处理应用程序时，它会在内部构建一个依赖图，此依赖图对应映射到项目所需的每个模块，并生成一个或多个bundle。

***

### 什么是CDN，它有什么作用

CDN，全称Content Delivery Network，即**内容分发网络**。

CDN是构建在网络之上的内容分发网络，依靠部署在各地的边缘服务器，通过中心平台的负载均衡、内容分发、调度等功能模块，**使用户就近获取所需内容，降低网络拥塞**，**提高用户访问响应速度和命中率**。

CDN的作用如下：

* **加速网站访问**：CDN可以将网站的静态资源（如图片、视频、CSS、JS等）缓存到离用户最近的节点上，当用户访问网站时，可以从离用户最近的节点获取这些资源，从而大大减少了加载时间，提高了网站的访问速度和用户体验。
...
> * 如果你正在定义一个对象的形状并需要默认值、继承或严格检查，应该使用接口（interface）。
> * 如果你只是想为某个类型提供一个有意义的名称或简化复杂的类型，应该使用类型别名（type）。

***

### <code>Generator</code>函数和<code>yield</code> 关键字

在JavaScript中，yield 关键字主要用于 Generator 函数（生成器函数）中。Generator 是一种特殊的函数，它允许你暂停和恢复函数的执行。yield 表达式用于返回函数当前的值，并暂停函数的执行，直到下一次调用该生成器的 next() 方法。
...
在上面的例子中，myGenerator 是一个 **Generator 函数，它使用 \* 符号进行标识**。在函数体内，我们使用 yield 关键字来返回值并暂停函数的执行。

当我们调用 myGenerator() 时，它不会立即执行函数体中的代码，而是返回一个生成器对象。我们可以使用这个生成器对象的 next() 方法来逐步执行函数并获取 yield 表达式的值。

每次调用 next() 方法时，生成器都会恢复执行，直到遇到下一个 yield 表达式或函数结束（如果使用了 return）。next() 方法返回一个对象，该对象有两个属性：value 和 done。value 属性是 yield 表达式的值（或函数的返回值），而 done 属性是一个布尔值，表示生成器是否已经完成执行。

Generator 和 yield 在许多场景中都很有用，特别是当你需要控制函数的执行流程时，例如异步编程、数据流处理或实现迭代器等。它们与 Promise 和 async/await 一起，为JavaScript提供了强大的异步编程能力。
...
它定义了如何遍历一系列值。**迭代器**有一个<code>next()</code>方法，每次调用这个方法时，它都会返回一个对象，这个对象有两个属性：<code>value</code>和<code>done</code>。

* value：当前迭代到的值。
* done：一个布尔值，表示是否还有更多的值可以迭代。如果所有的值都被迭代过了，这个值为true。

**iterable**

可以**被迭代**的**对象**，有一个特殊的内部方法（称为\[**Symbol.iterator**]），该方法返回一个迭代器。大多数内置的可迭代对象（如**数组、字符串、Map、Set等）都是可迭代的**。

例如，数组是可迭代的，因为它们有一个\[Symbol.iterator]方法，返回一个迭代器。这意味着你可以使用**for...of循环**遍历数组的元素。
...
下面是一个简单的示例，展示如何使用可迭代的对象（如数组）和迭代器：

首先获取了数组的迭代器，然后使用next()方法逐个获取值，直到所有的值都被遍历过，此时done属性为true。
...
* **独立运行**时\ 每个微应用之间状态隔离，运行时状态不共享

***

### 定时器为什么是不精确的

1. **事件循环\*\*\*\*（Event Loop）**：JavaScript 是单线程的，它使用事件循环来处理异步操作。当你在 JavaScript 中设置一个定时器，它实际上是进入了一个队列，等待事件循环处理。但是，事件循环中的任务并不会立即执行，而是根据任务的优先级和可用的 CPU 时间来决定何时执行。因此，定时器的执行时间并不是精确的。
2. **操作系统\*\*\*\*的调度**：JavaScript 的定时器是基于浏览器的计时器 API，而这些 API 实际上是依赖于操作系统的计时功能。操作系统的计时器并不保证绝对的精确性，它会受到操作系统调度的影响。
3. **浏览器事件处理**：浏览器在处理其他事件（如用户交互、网络请求等）时，可能会影响定时器的执行。例如，当用户与页面交互时，浏览器可能会优先处理这些事件，从而延迟了定时器的执行。
4. **其他代码的执行**：如果你的代码中还有其他异步操作（如网络请求、setTimeout 等），它们可能会与定时器重叠，导致定时器的执行时间进一步偏离预期。
...
虽然在不设定 delay 延时时间的情况下，都是默认延时 **0 毫秒**执行，但是 setTimeout 有一个显著的缺陷在于**时间是不精确的**：setTimeout/setInterval定时器 只能保证延时或者间隔不小于设定的时间。因为它们实际上只是**把任务添加到了任务队列中**，但是如果前面的任务还没有执行完成，它们必须要等待。
...
宏任务是指那些**需要较长时间才能完成的任务**，比如**定时器、事件监听器、I/O操作、UI渲染**等。这些任务在事件循环中被视为单独的单元，每次循环只处理一个宏任务。

* **微任务（MicroTask）**：

微任务是指那些**执行时间较短的任务**，比如**Promise、process.nextTick、MutationObserver**等。这些任务在事件循环的每个阶段都可能被处理，甚至可以中断当前正在执行的宏任务来处理微任务。

***

### for.in 和 object.keys的区别

for...in 和 Object.keys() 在 JavaScript 中都可以用来**遍历对象的属性**，但它们之间有几个重要的区别。

![1717574996099-e649791f-5c01-4dc5-8146-3cf3c617005a.png](/assets/img/posts/知识积累/面试题/1717574996099-e649791f-5c01-4dc5-8146-3cf3c617005a-476605.png)

1. **遍历的属性**：
   * for...in 循环不仅遍历对象自身的可枚举属性，**还会遍历其****原型链****上的可枚举属性**。

> 如果你只想遍历对象自身的属性，那么你需要添加一个检查，例如使用 <code>hasOwnProperty()</code> 方法。

```
- Object.keys() 返回一个由对象自身的所有可枚举属性（不包括继承的）组成的数组。
```

2\. **空值处理**：
\- for...in 循环不会忽略空值或undefined属性。
\- Object.keys() 会忽略值为undefined的属性。
3\. **性能**：
\- 对于大量的数据，使用 Object.keys() 可能比 for...in 循环更快，因为它不需要遍历原型链。
4\. **兼容性**：
\- for...in 是 JavaScript 的基础语法，所以它应该在所有 JavaScript 环境中都能工作。
\- Object.keys() 是 ES5 的新特性，一些较老的 JavaScript 引擎可能不支持它。
5\. **返回值**：
\- for...in 直接返回一个对象的属性名。
\- Object.keys() 返回一个包含对象自身可枚举属性的数组。
6\. **使用场景**：
\- 如果需要遍历对象的所有可枚举属性（包括自身和原型链上的），那么 for...in 可能是更好的选择。
\- 如果只需要遍历对象自身的可枚举属性，并且想要一个数组来使用，那么 Object.keys() 可能是更好的选择。

总结：在大多数情况下，如果你只需要遍历对象自身的可枚举属性，并且想要一个数组来使用，那么 Object.keys() 是更好的选择。如果你需要遍历对象的所有可枚举属性（包括原型链上的），那么 for...in 是更好的选择。
...
> history 提供了两个方法，能够无刷新的修改用户的浏览记录；
> pushState 在用户访问页面后面添加一个访问记录， replaceState 则是直接替换了当前访问记录

1. **通过 hash(#) 哈希实现 - \*\*\*\*hashchange**

过监听 hash(#) 的变化来执行js代码 从而实现 页面的改变      
...
2. **通过html5的最新history api来实现**

只要刷新 这个url（www.ff.ff/jjkj/fdfd/fdf/fd）就会请求服务器，然而服务器上根本没有这个资源，所以就会报404，解决方案就 配置一下服务器端；
...
* history.state
* window.**onpopstate**事件 - 主要是监听浏览器前进后退的按钮，不能触发pushState、replaceState
...
1. <code>localStorage</code>方式 - <code>setItem</code>时会自动触发整个浏览器的storage事件（除当前页面之外）
...
2. <code>webSocket</code>方式 - a.html发送消息到WebSocketServer，WebSocketServer再实时把消息发给 b.html

![1709100596653-2b7360f1-b909-46a7-84cb-ab1a94aa12a6.png](/assets/img/posts/知识积累/面试题/1709100596653-2b7360f1-b909-46a7-84cb-ab1a94aa12a6-811678.png)
...
* 这个事件会在整个页面完全加载后触发，包括所有的图像、样式表、脚本等。
* 这意味着，如果一个页面有很多图片或其他外部资源，window.onload 可能会延迟很长时间才触发。
* 它是一个纯 JavaScript 的事件，不依赖于任何库。
...
* 如果你在使用 jQuery，$(document).ready() 会在 DOM（Document Object Model）结构加载完成后触发，而不必等待所有的外部资源如图片和样式表加载完成。
* 这意味着，你可以更快地执行与 DOM 相关的操作，而不必等待所有的外部资源都加载完毕。
* 这个方法是 jQuery 提供的，所以你需要先引入 jQuery 库。
